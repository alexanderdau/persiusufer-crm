...
Deno.serve(async (req) => {
  ...
  if (!akte) return new Response(renderError("Verfahren nicht gefunden."), { status: 404, headers: { "Content-Type": "text/html; charset=utf-8" } });

  if (req.method === "GET") {
    await sb.from("zvg_anfrage").update({ reply_form_views_count: (anfrage.reply_form_views_count ?? 0) + 1, reply_form_first_viewed_at: anfrage.reply_form_first_viewed_at ?? new Date().toISOString() }).eq("id", anfrage.id);
    return new Response(renderForm(akte, anfrage, token), { headers: { "Content-Type": "text/html; charset=utf-8" } });
  }

  if (req.method === "POST") {
    const form = await parseForm(req);
    const csrf = form.get("csrf");
    if (csrf !== token) return new Response(renderError("Sicherheitsprüfung fehlgeschlagen."), { status: 403, headers: { "Content-Type": "text/html; charset=utf-8" } });
    const optionRaw = form.get("option");
    const option = optionRaw ? parseInt(optionRaw, 10) : null;
    if (!option || option < 1 || option > 8) return new Response(renderForm(akte, anfrage, token, "Bitte eine Option auswählen."), { status: 400, headers: { "Content-Type": "text/html; charset=utf-8" } });

    const update = { antwort_eingegangen_am: new Date().toISOString(), antwort_option: option, status: "beantwortet", reply_token_used_at: new Date().toISOString() };
    if (option === 1 || option === 3) {
      const d = parseDeDate(form.get(option === 1 ? "termin_1" : "termin_3"));
      const u = form.get(option === 1 ? "uhrzeit_1" : "uhrzeit_3");
      const saal = form.get(option === 1 ? "saal_1" : "saal_3");
      if (d) update.antwort_neuer_termin = composeTimestamp(d, u);
      if (saal) update.antwort_neuer_termin_saal = saal;
    }
    if (option === 2) { const d = parseDeDate(form.get("termin_2")); if (d) update.antwort_neuer_termin = composeTimestamp(d); }
    if (option === 4) update.antwort_verfahren_eingestellt = true;
    if (option === 5) update.antwort_zuschlag_im_termin = true;
    if (option === 6) { update.antwort_zuschlag_versagt = true; const d = parseDeDate(form.get("termin_6")); if (d) update.antwort_neuer_termin = composeTimestamp(d); }
    if (option === 7) { const d = parseDeDate(form.get("termin_7")); if (d) update.antwort_verteilungstermin = d + "T00:00:00"; }
    if (option === 8) { const ft = form.get("freitext"); if (ft) update.antwort_freitext = ft.toString().slice(0, 4000); }
    const senderName = form.get("sender_name"); const senderRole = form.get("sender_role");
    ...
    if (error) return new Response(renderError(`Speichern fehlgeschlagen: ${error.message}`), { status: 500, headers: { "Content-Type": "text/html; charset=utf-8" } });
    return new Response(renderSuccess(akte, option), { headers: { "Content-Type": "text/html; charset=utf-8" } });
  }
